...
export default {
  ...
  setup() {
    ...
    const initKlineChart = () => {
      ...
      const apiUrl = `http://localhost:8088/api/v2/graph`;
...
.then(response=>{
  ...
  .then(data => {
    ...
    // Extract data
    categoryData.push([data.localDateTime]); // Convert timestamps to date strings
    values.push([data.open, data.close, data.high, data.low]);
    vols.push(data.volumn);
...
const interval = 10000; // 100 seconds
